...
static bool find_cgroup_subsystems(char ***kernel_subsystems)
{
	...
	proc_cgroups = lxc_fopen_cloexec("/proc/cgroups", "r");
	if (!proc_cgroups)
		return false;
...
static bool find_hierarchy_mountpts( struct cgroup_meta_data *meta_data, char **kernel_subsystems)
{
	...
	proc_self_mountinfo = lxc_fopen_cloexec("/proc/self/mountinfo", "r");
	/* if for some reason (because of setns() and pid namespace for example),
	 * /proc/self is not valid, we try /proc/1/cgroup... */
	if (!proc_self_mountinfo)
		proc_self_mountinfo = lxc_fopen_cloexec("/proc/1/mountinfo", "r");
	if (!proc_self_mountinfo)
		return false;
...
out:
	lxc_fclose(proc_self_mountinfo);
	free(tokens);
	free(line);
	return bret;
}
...
struct cgroup_process_info *lxc_cgroup_process_info_getx(const char *proc_pid_cgroup_str, struct cgroup_meta_data *meta)
{
	...
	proc_pid_cgroup = lxc_fopen_cloexec(proc_pid_cgroup_str, "r");
	if (!proc_pid_cgroup)
		return NULL;
...
bool cgroup_devices_has_allow_or_deny(struct lxc_handler *h, char *v, bool for_allow)
{
	...
	devices_list = lxc_fopen_cloexec(path, "r");
	if (!devices_list) {
		free(path);
		return false;
	}
