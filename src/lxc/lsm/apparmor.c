...
"%s" // AA_PROFILE_DENY_PROC_SYS_PRE_6_7 OR AA_PROFILE_DENY_PROC_SYS_POST_6_7
...
static const char AA_PROFILE_BINFMT_MISC_MOUNT[] =
"  # deny writes in /proc/sys/fs but allow binfmt_misc to be mounted\n"
"  mount fstype=binfmt_misc -> /proc/sys/fs/binfmt_misc/,\n"
"  deny @{PROC}/sys/fs/** wklx,\n"
;

static const char AA_PROFILE_DENY_PROC_SYS[] =
"  deny /proc/sys/[^kn]*{,/**} wklx,\n"
;

static const char AA_PROFILE_BINFMT_MISC_MOUNT_RW_6_7[] =
"  # allow binfmt_misc to be mounted with read-write permissions\n"
"  mount fstype=binfmt_misc options=(rw) -> /proc/sys/fs/binfmt_misc/,\n"
;

static const char AA_PROFILE_DENY_PROC_SYS_6_7[] =
"  deny /proc/sys/[^knf]*{,/**} wklx,\n"
...
/* As of linux 6.7 binfmt allows kernel namespaces */
static bool apparmor_binfmt_misc_ns_allowed(void)
{
	const char *min_version = "6.7.0";
	return lxc_check_kernel_version_met(min_version);
}
...
{
	__do_free char *base_profile = NULL;
	__do_free char *base_profile_no_nesting = NULL;
	char *profile, *profile_name_full;
	const char *binfmt_misc_mount_rule;
	const char *deny_proc_sys_rule;
	int ret;
	size_t size;
...
"profile \"", profile_name_full, "\" flags=(attach_disconnected,mediate_deleted) {\n",
	...
	if (apparmor_binfmt_misc_ns_allowed()) {
		log_info(0, "AppArmor Binfmt Misc Namespace is allowed");
		binfmt_misc_mount_rule = AA_PROFILE_BINFMT_MISC_MOUNT_RW_6_7;
		deny_proc_sys_rule = AA_PROFILE_DENY_PROC_SYS_6_7;
	} else {
		binfmt_misc_mount_rule = AA_PROFILE_BINFMT_MISC_MOUNT;
		deny_proc_sys_rule = AA_PROFILE_DENY_PROC_SYS;
	}

	ret = snprintf(NULL, 0, AA_PROFILE_BASE, binfmt_misc_mount_rule,
		       deny_proc_sys_rule);
	if (ret < 0)
		return log_error_errno(NULL, EIO, "Failed to size AppArmor profile template");

	base_profile = zalloc((size_t)ret + 1);
	if (!base_profile)
		return log_error_errno(NULL, ENOMEM, "Failed to allocate memory for AppArmor profile template");

	ret = snprintf(base_profile, (size_t)ret + 1, AA_PROFILE_BASE,
		       binfmt_misc_mount_rule, deny_proc_sys_rule);
	if (ret < 0)
		return log_error_errno(NULL, EIO, "Failed to generate AppArmor profile template");

	must_append_sized(&profile, &size, base_profile, strlen(base_profile));
	if (!conf->lsm_aa_allow_nesting) {
		ret = snprintf(NULL, 0, AA_PROFILE_BASE_NO_NESTING,
			       deny_proc_sys_rule);
		if (ret < 0)
			return log_error_errno(NULL, EIO, "Failed to size AppArmor no-nesting profile template");

		base_profile_no_nesting = zalloc((size_t)ret + 1);
		if (!base_profile_no_nesting)
			return log_error_errno(NULL, ENOMEM, "Failed to allocate memory for AppArmor no-nesting profile template");

		ret = snprintf(base_profile_no_nesting, (size_t)ret + 1,
			       AA_PROFILE_BASE_NO_NESTING, deny_proc_sys_rule);
		if (ret < 0)
			return log_error_errno(NULL, EIO, "Failed to generate AppArmor no-nesting profile template");

		must_append_sized(&profile, &size, base_profile_no_nesting,
			          strlen(base_profile_no_nesting));
	}
