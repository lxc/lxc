...
"profile \"", profile_name_full, "\" flags=(attach_disconnected,mediate_deleted) {\n",
	...
	char *base_profile = zalloc(sizeof(AA_PROFILE_BASE) 
								+ sizeof(AA_PROFILE_DENY_PROC_SYS_6_7) 
	...
	if (apparmor_binfmt_misc_ns_allowed()) {
		...
		sprintf(base_profile, AA_PROFILE_BASE,
				AA_PROFILE_BINFMT_MISC_MOUNT_RW_6_7,
				AA_PROFILE_DENY_PROC_SYS_6_7);
		}else {
		
		sprintf(base_profile,
				AA_PROFILE_BASE,
				AA_PROFILE_BINFMT_MISC_MOUNT,
				AA_PROFILE_DENY_PROC_SYS);
	}

	must_append_sized(&profile, &size, base_profile,
	                  strnlen(base_profile, sizeof(AA_PROFILE_BASE) 
										  + sizeof(AA_PROFILE_DENY_PROC_SYS_6_7) 
										  + sizeof(AA_PROFILE_BINFMT_MISC_MOUNT) + 1));
	free(base_profile);
