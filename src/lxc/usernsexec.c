...
static int add_map_entry(struct list_head *active_map, long host_id, long ns_id, long range, int which)
{
 ...
 * to 100010
 */
static int parse_map(struct list_head *active_map, char *map)
{
	int i, ret, idtype;
	...
	for (i = 0; i < 2; i++) {
		...
		ret = add_map_entry(active_map, host_id, ns_id, range, idtype);
		if (ret < 0)
			return ret;
	}
 ...
 */
static int read_default_map(struct list_head *active_map, char *fnam, int which, char *user)
{
	...
	while (getline(&line, &sz, fin) != -1) {
		...
		*p2 = '\0';
		...
		ret = add_map_entry(active_map, ul1, 0, ul2, which);
		break;
	}
...
static int find_default_map(struct list_head *active_map)
{
	...
	ret = read_default_map(active_map, subuidfile, ID_TYPE_UID, pwent.pw_name);
	if (ret < 0)
		return -1;

	ret = read_default_map(active_map, subgidfile, ID_TYPE_GID, pwent.pw_name);
	if (ret < 0)
		return -1;
...
static bool do_map_self(struct list_head *active_map)
{
	struct id_map *map;
	...
	ret = add_map_entry(active_map, getgid(), nsgid, 1, ID_TYPE_GID);
	if (ret < 0)
		return false;
	ret = add_map_entry(active_map, getuid(), nsuid, 1, ID_TYPE_UID);
	if (ret < 0)
		return false;
	return true;
}

int lxc_usernsexec(int argc, char *argv[], int *status)
{
	int c, pid, ret;
	char buf[1];
	...
	while ((c = getopt(argc, argv, "m:hs")) != EOF) {
		switch (c) {
		case 'm':
			ret = parse_map(&active_map, optarg);
			if (ret < 0)
				return -1;
			break;
		case 'h':
	...
	if (list_empty(&active_map)) {
		ret = find_default_map(&active_map);
		if (ret < 0) {
