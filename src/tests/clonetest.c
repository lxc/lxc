...
int main(int argc, char *argv[])
{
	...
	c2 = c->clone(c, MYNAME2, NULL, 0, NULL, NULL, NULL);
	if (!c2) {
	...
	if ((c2 = c->clone(c, "clonetestlvm2", NULL, 0, NULL, NULL, NULL)) == NULL) {
	...
	if ((c2 = c->clone(c, "clonetestlvm3", NULL, LXC_CLONE_SNAPSHOT, NULL, NULL, NULL)) == NULL) {
		fprintf(stderr, "lvm clone failed\n");
		goto out;
	}
	lxc_container_put(c2);
	...
	if ((c2 = c->clone(c, "clonetest-o1", NULL, LXC_CLONE_SNAPSHOT, "overlayfs", NULL, NULL)) == NULL) {
		fprintf(stderr, "overlayfs clone of dir failed\n");
		goto out;
	}

	// Now create an overlayfs clone of the overlayfs clone
	if ((c3 = c2->clone(c2, "clonetest-o2", NULL, LXC_CLONE_SNAPSHOT, "overlayfs", NULL, NULL)) == NULL) {
